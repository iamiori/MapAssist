...
namespace MapAssist.Types
{
    public class UnitAny : IUpdatable<UnitAny>
    {
                        ...
                        switch (_unitAny.UnitType)
                        {
                            ...
                            case UnitType.Monster:
                                if (IsMonster())
                                {
                                    ...
                                    //if(MapAssistConfiguration.Loaded.ShowUnitInfo.Enabled)
                                    //{
        ...
        public string ioriGetStat => _ioriGetStat;
        ...
        public bool IsOnShopWindows()
        {
            var itemMode = (ItemMode)_unitAny.Mode;
            var itemModeLoc = (BodyLoc)_itemData.BodyLoc;
            return false;
        ...
        {
            var itemMode = (ItemMode)_unitAny.Mode;
            return itemMode == ItemMode.ONCURSOR;
        }
        ...
        private string iori_GetStatInfo()
        {
            //_statList.TryGetValue(Stat.STAT_VITALITY, out var _itemlv);
            //return _itemlv.ToString();
            var tmpString = "";
            //return tmpString;

            tmpString +=   "\n";

            foreach (var aa in _statList)
            {
                tmpString += aa.Key.ToString() + "(" + (int)aa.Key + ") = " + aa.Value.ToString() + "\n";
