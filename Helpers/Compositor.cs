...
namespace MapAssist.Helpers
{
    public class Compositor : IDisposable
    {
        ...
        public bool isState(Types.UnitAny _unit ,State id)
        {
            foreach (var state in _unit.StateList)
            {
                if (state == id)
                {
                    return true;
            ...
            foreach (var mobRender in monsterRenderingOrder)
            {
                foreach (var unitAny in _gameData.Monsters)
                {
                    if (mobRender == GetMonsterIconRendering(unitAny.MonsterData) && mobRender.CanDrawIcon())
                    {
                        ...
                        if(isState(unitAny, State.STATE_REVIVE))
                        {
                            isReviveMonster = true;
                        }
                        if (NPC.Sums.TryGetValue(unitAny.TxtFileNo, out var _))
                        {
                            isReviveMonster = true;
                        }
                        if (isReviveMonster)
                        ...
                        {
                            System.Diagnostics.Debug.WriteLine(unitAny.TxtFileNo+" , " + unitAny.Name);
                ...
                foreach (var player in _gameData.Roster.List)
                {
                    ...
                    if (_gameData.Players.TryGetValue(player.UnitId, out var playerUnit))
                    {
                        ...
                        if (inMyParty && player.PartyID < ushort.MaxValue) // partyid is max if player is not in a party
                        {
                            ...
                            {
                                DrawText(gfx, MapAssistConfiguration.Loaded.MapConfiguration.Player, playerUnit.Position, "", 
        ...
        public void DrawItemLog(Graphics gfx, Point anchor)
        {
            ...
            for (var i = 0; i < ItemLog.Length; i++)
            {
                ...
                var itemSpecialName = "";
                var itemLabelExtra = "";
                var onCURSOR = (ItemMode)item.Mode == ItemMode.ONCURSOR && MapAssistConfiguration.Loaded.MouseItemInfo.Enabled;

                //if(ItemClass._itemClass[Items.ItemName(item.TxtFileNo)]["Class"] != null)
                //{
                //    itemLabelExtra += "[" + ItemClass._itemClass[Items.ItemName(item.TxtFileNo)]["Class"] + "]";
                //}

                if ((ItemMode)item.Mode == ItemMode.ONCURSOR)
                {
                    itemLabelExtra += "[OnCursor]" + "(" + item.TxtFileNo + ")";
                }
                else
                {
                    itemLabelExtra += "(" + item.TxtFileNo + ")";
                }
                ...
                if (isEth)
                {
                    ...
                    {
                        if (ItemLog[i].ItemData.ItemQuality < ItemQuality.MAGIC)
                ...
                if (ItemClass._itemClass[Items.ItemName(item.TxtFileNo)] != null)
                {
                    if (ItemClass._itemClass[Items.ItemName(item.TxtFileNo)]["Type"].ToString() == "Rune")
                    {
                ...
                {
                    itemLabelExtra += "\n";
                    itemLabelExtra += item.ioriGetStat;
                }
                var newname = itemBaseName;

                if(ItemClass._itemClass[Items.ItemName(item.TxtFileNo)] != null)
                {
                    newname = itemBaseName +" [" + ItemClass._itemClass[Items.ItemName(item.TxtFileNo)]["CN"] + "]";
                    if(ItemClass._itemClass[Items.ItemName(item.TxtFileNo)]["Type"].ToString() == "Paladin Shields")
                    {
                        if(item.Stats.TryGetValue(Stat.STAT_FIRERESIST, out var fireres))
                        {
                            newname += "[Res:" + fireres + "]";
                        }
                        else
                    ...
                    if (item.Stats.TryGetValue(Stat.STAT_ARMORCLASS, out var _def))
                    {
                        newname += "[Def:" + _def + "]";
                    }
